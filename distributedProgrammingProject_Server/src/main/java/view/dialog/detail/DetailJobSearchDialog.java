...
public class DetailJobSearchDialog extends JDialog implements ActionListener{
	...
	ComboBoxMultiSelection professionBox;

	public DetailJobSearchDialog(Frame parent, boolean modal, Job job, Employee employee) {
		super(parent, modal);
...
//	public DetailJobSearchDialog(Frame parent, boolean modal, Job job, Resume resume, Employee employee, boolean check) {
//		this(parent, modal, job, resume, employee);
//		btnPanel.removeAll();
//
...
//		Thông tin việc làm
		...
		professionBox = new ComboBoxMultiSelection();
		professionBox.setModel(new DefaultComboBoxModel<>(
				professions.stream()
		...
		professionBox.setEnabled(false);
...
//		Button
		...
		btnApply.setPreferredSize(new Dimension(120,25));
...
//		descriptionText.setText(ttd.getMoTa());
	...
	public void ungtuyen() {
        JOptionPane.showMessageDialog(rootPane, "Apply for job");
